...
function NewDestination() {
    ...
    const list = destinations.map((destination:any) => {
    ...
    function addSet(name:any) {
        checkList.add(name)
    }

    //ADD NEW(POST) TO THE DB DESTINATION FUNCTION 
    const addDestination = async (e:any) => {
        e.preventDefault()
        try {
        ...
        <>
            ...
            <Form method='POST' onSubmit={addDestination} >
                ...
                <Form.Group className="mb-3">
                    ...
                    <DatePicker  onChange={(date_visited:any) => setDate_visited(date_visited)}  />
                </Form.Group>
